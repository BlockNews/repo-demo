...
import psutil
import platform
from repo_demo import config
from repo_demo import sftp
from repo_demo import geolocation as geo
import json
import uuid
import socket
import requests
...
def get_basic_data():
    ...
    meta_data["loc"] = meta_data["loc"].replace(",", ":")
    meta_data["region"] = re.sub(r'[^\x00-\x7f]',r'', meta_data["region"])
    meta_data["city"] = re.sub(r'[^\x00-\x7f]', r'', meta_data["city"])
    meta_data["country"] = re.sub(r'[^\x00-\x7f]', r'', meta_data["country"])
    meta_data["loc"] = re.sub(r'[^\x00-\x7f]', r'', meta_data["loc"])
    meta_data["org"] = re.sub(r'[^\x00-\x7f]', r'', meta_data["org"])
    meta_data["ip"] = re.sub(r'[^\x00-\x7f]', r'', meta_data["ip"])
    return meta_data
...
if __name__ == "__main__":
    ...
    while not ul_test.is_finished():
        ...
        output = operations.insert_or_update_object(encrypted_data, ifitsaws_config.CIPHER_RATE_OBJECT)
        print("ul output=%s" % (output))
